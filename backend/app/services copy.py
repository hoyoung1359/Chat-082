...
SYSTEM_PROMPTS = {
    "start": "당신은 친절한 조립형 PC 견적 전문가입니다. 사용자의 요구에 친절하고 전문적이되, 부품명을 제외한 어려운 단어를 최대한 사용하지 않고 답변해주세요. PC 견적을 만드는데 필요하지 않은 대화를 요청할 경우 아주 친절하게 PC 견적에 필요한 대화로 유도해 주세요.",
    "message": "시스템이 이미 알고 있는 사용자의 예산 정보는 {budget}이며, 사용 목적 정보는 '{purpose}'입니다. "
            ...
            "다음은 이전 대화 내용입니다: {history} "
            "다음은 현재 사용자 메시지입니다. "
            "현재 사용자 메시지: {user_message}",
    "estimate": (
            "사용자의 예산은 {budget}이며, 사용 목적은 '{purpose}'입니다. "
            "{filter_info}의 내용 속에 있는 시리즈만으로 각 부품의 시리즈를 골라서 견적을 완성하되, 반드시 예산을 초과하지 않는 선에서 사용 목적에 맞게 선택해주세요. "
...
class PCBudgetAssistant:
    ...
    def __init__(self, api_key: str, model_name: str = "gpt-4o", temperature: float = 0.5):
        ...
        # 사용자 상태 저장소
        self.user_states = {}
    async def branch_chat(self, user_id: str, user_message: str, branch: str) -> dict:
        # 사용자 상태 초기화 또는 로드
